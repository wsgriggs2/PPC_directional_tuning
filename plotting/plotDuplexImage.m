...
% cmap = plotDuplexImage(X_img_mm, Z_img_mm, statistical_overlay, angiogram);
%
%
% vargin:
% -----------------------------------------------------------
...
% colorbar_ticks        (vector, default [0, 0.33 0.66, 1])
...
p.addParameter('displayStrongest', false);
p.addParameter('transparencyMap', 1);
p.addParameter('ColorBarTitle', '');
p.addParameter('darkestShade', 0);
p.addParameter('data_aspect_ratio', [1 1 1]);
p.addParameter('colorbar_ticks', [0, 0.33, 0.66, 1])
p.parse(varargin{:});
arg = p.Results;


if arg.displayStrongest
if arg.displayStrongest==1, arg.displayStrongest = 0.1; end
threshold = quantile(abs(foreground(:)), 1-arg.displayStrongest);
if ~isnan(threshold)
    foreground(abs(foreground)<threshold) = nan;
end
end
...
if arg.displayStrongest && (arg.displayBot || arg.displayBest || arg.displayTop)
   error('Currently cannot use `displayStrongest` with the other quantile options. Fix!'); 
end

% displayBest, displayTop, displayBot
if arg.displayBot 
    arg.displayBest = arg.displayBot; 
end
% handle errors
...
%     end
    ...
    if any(~isnan(Ifg), 'all')
        image_fg = Ifg;
        image_fg(1, 1) = 1; % Pixel to sack for visualization purposes
...
% colorbar
if arg.showColorbar
    cbn = round(linspace(fgm,fgM,length(arg.colorbar_ticks)),5);
    cb = colorbar('Ticks',arg.colorbar_ticks,...
        'Ticklabels', num2str(cbn'));
    set(cb,'YLim',[0 1])
