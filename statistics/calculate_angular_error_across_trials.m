...
%% Generates mean angular error across trials and also generates null distribution for angular error
% Inputs:
...
%                                 replicate
%
% Written by Whitney Griggs May 21, 2022
...
for trial = 1:nTrials
    ...
    if isnan(inputs.rolling_average)
        ...
        pvalue(trial) = 1 - nnz(null_mean_angular_error(:, trial) > empiric_mean_angular_error(trial))/inputs.num_repetitions;
    else
        ...
        % Calculate percentage of null distribution exceeding true value
        pvalue(trial) = 1 - nnz(null_mean_angular_error(:, min(trial, inputs.rolling_average)) > empiric_mean_angular_error(trial))/inputs.num_repetitions;
    end
end
