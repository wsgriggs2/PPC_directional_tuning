...
classdef scalebar <handle
	...
	methods
		function hobj = scalebar(varargin)                        
			...
			templine = plot([0 0],[0 0],'Parent',hAxes,'Color','k','LineWidth',4);
			...
			hobj.hTextX = text(0,0,'','Parent',hAxes,'ButtonDownFcn',@hobj.FcnStartDrag,'FontSize',18);
            ...
            %Parse Param-Value
            p = inputParser;
            p.addParameter('Position',[axisXLim(1) + 0.05*axisXWidth, axisYLim(1) + 0.05*axisYWidth]);
            ...
            p.addParameter('YLen', 0);%nearto(0.1*axisYWidth));
            p.addParameter('hTextX_Pos',[-0.03*axisXWidth, 0.06*axisYWidth]);
            p.addParameter('hTextY_Pos',0.05*[-axisXWidth, axisYWidth]);
